...
from user.tests.helper.factories import ImageFactory, ProfileFactory, UserFactory
...
class UserProfileModelTest(TestCase):
    ...
    def test_full_clean_passes_when_profile_picture_has_valid_extension(self):
        ...
        for ext, fmt, content_type in [
            ...
            with self.subTest(ext=ext):
                ...
                profile.profile_picture = ImageFactory.create(
                    name=f"test.{ext}", format=fmt, content_type=content_type
                )
                profile.full_clean()
    ...
    def test_str_returns_user_first_and_last_names_and_role(self):
        self.assertEqual(
            str(self.profile),
            f"{self.profile.user.first_name} {self.profile.user.last_name} ({self.profile.role})",
        )
