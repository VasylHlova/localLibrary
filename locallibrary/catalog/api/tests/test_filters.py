...
def overdue_test_data():
    user = UserFactory()
    inst_active_overdue = BookInstanceFactory(
        status=InstanceStatus.ON_LOAN, borrower=user, due_back=date.today() - timedelta(days=2)
    )
    loan_active_overdue = LoanFactory(book_instance=inst_active_overdue, borrower=user, returned_at=None)
    inst_active_ok = BookInstanceFactory(
        status=InstanceStatus.ON_LOAN, borrower=user, due_back=date.today() + timedelta(days=2)
    )
    loan_active_ok = LoanFactory(book_instance=inst_active_ok, borrower=user, returned_at=None)
    inst_returned_overdue = BookInstanceFactory(
        status=InstanceStatus.AVAILABLE, borrower=None, due_back=date.today() - timedelta(days=5)
    )
    loan_returned_overdue = LoanFactory(
        book_instance=inst_returned_overdue, borrower=user, returned_at=date.today() - timedelta(days=2)
    )
    inst_returned_ok = BookInstanceFactory(
        status=InstanceStatus.AVAILABLE, borrower=None, due_back=date.today() - timedelta(days=2)
    )
    loan_returned_ok = LoanFactory(
        book_instance=inst_returned_ok, borrower=user, returned_at=date.today() - timedelta(days=5)
    )
    return {
