...
class AuthorCreateViewTest(TestCase):
    def setUp(self):
        ...
        test_user1 = User.objects.create_user(username="test_user1", password="some_password1")
        test_user2 = User.objects.create_user(username="test_user2", password="some_password2")
    ...
    def test_form_date_of_death_initially_set_to_expected_date(self):
        ...
        expected_initial_date = datetime.date(2020, 12, 31)
    ...
    def test_add_author_with_invalid_data(self):

        self.client.login(username="test_user1", password="some_password1")
        first_name = "Test"
        last_name = "Test"
        date_of_birth = datetime.date(1999, 1, 1)
        invalid_date_of_death = datetime.date(1998, 12, 12)
        ...
        response = self.client.post(
            reverse("author-create"),
            {
                ...
                "date_of_death": invalid_date_of_death,
            },
        )

        self.assertEqual(response.status_code, 200)
        self.assertFormError(
            response.context["form"], "date_of_death", "Author could not die earlier than was born!!!"
        )

    def test_add_author_with_valid_data(self):

        self.client.login(username="test_user1", password="some_password1")
        first_name = "Test2"
        last_name = "Test2"
        date_of_birth = datetime.date(1999, 1, 1)
        invalid_date_of_death = datetime.date(1999, 12, 12)
